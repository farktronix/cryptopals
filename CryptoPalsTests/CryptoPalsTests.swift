...
class CryptoPalsTests: XCTestCase {
    ...
    func testChallenge6() {
        ...
        if let cyphertextPath = testBundle.path(forResource: "Challenge6", ofType: "txt", inDirectory: nil) {
            do {
                ...
                XCTAssertEqual(decryptedString[decryptedString.startIndex..<decryptedString.index(decryptedString.startIndex, offsetBy: 33)], "I'm back and I'm ringin' the bell")
            } catch {
                XCTFail("Couldn't read from the cyphertext file at \(cyphertextPath)")
            }
        }
    ...
    func testChallenge7() {
        let testBundle = Bundle(for: type(of: self))
        if let cyphertextPath = testBundle.path(forResource: "Challenge7", ofType: "txt", inDirectory: nil) {
            do {
                let fileCyphertextString = try String(contentsOfFile: cyphertextPath).replacingOccurrences(of: "\n", with: "")
                let cyphertext : [UInt8] = base64Decode(b64String: fileCyphertextString)
                let key : [UInt8] = Array("YELLOW SUBMARINE".utf8)
                let plaintext = AESDecryptECB128(cyphertext: cyphertext, key: key)
                let asciiString = String(ascii: plaintext!)
                XCTAssertEqual(asciiString[asciiString.startIndex..<asciiString.index(asciiString.startIndex, offsetBy: 33)], "I'm back and I'm ringin' the bell")
            } catch {
                XCTFail("Couldn't read from the cyphertext file at \(cyphertextPath)")
            }
        }
    ...
    // Use example values from C.1 in https://csrc.nist.gov/csrc/media/publications/fips/197/final/documents/fips-197.pdf
    func testAESEncryptC1() {
        let plaintext : [UInt8] = [0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff]
        let expectedCyphertext : [UInt8] = [0x69, 0xc4, 0xe0, 0xd8, 0x6a, 0x7b, 0x04, 0x30, 0xd8, 0xcd, 0xb7, 0x80, 0x70, 0xb4, 0xc5, 0x5a]
    ...
    // Use example values from C.1 in https://csrc.nist.gov/csrc/media/publications/fips/197/final/documents/fips-197.pdf
    func testAESDecryptC1() {
        let cyphertext : [UInt8] = [0x69, 0xc4, 0xe0, 0xd8, 0x6a, 0x7b, 0x04, 0x30, 0xd8, 0xcd, 0xb7, 0x80, 0x70, 0xb4, 0xc5, 0x5a]
        let expectedPlaintext : [UInt8] = [0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff]
        let key : [UInt8] = [0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f]
        let plaintext = AESDecryptBlock128(cyphertext: cyphertext, key: key)
    ...
    func testAESDecryptBlock() {
        let cyphertext : [UInt8] = [0x3A, 0xD7, 0x7B, 0xB4, 0x0D, 0x7A, 0x36, 0x60, 0xA8, 0x9E, 0xCA, 0xF3, 0x24, 0x66, 0xEF, 0x97, 0xF5, 0xD3, 0xD5, 0x85, 0x03, 0xB9, 0x69, 0x9D, 0xE7, 0x85, 0x89, 0x5A, 0x96, 0xFD, 0xBA, 0xAF, 0x43, 0xB1, 0xCD, 0x7F, 0x59, 0x8E, 0xCE, 0x23, 0x88, 0x1B, 0x00, 0xE3, 0xED, 0x03, 0x06, 0x88, 0x7B, 0x0C, 0x78, 0x5E, 0x27, 0xE8, 0xAD, 0x3F, 0x82, 0x23, 0x20, 0x71, 0x04, 0x72, 0x5D, 0xD4]
        let key : [UInt8] = [0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c]
        let expectedPlaintext : [UInt8] = [0x6B, 0xC1, 0xBE, 0xE2, 0x2E, 0x40, 0x9F, 0x96, 0xE9, 0x3D, 0x7E, 0x11, 0x73, 0x93, 0x17, 0x2A, 0xAE, 0x2D, 0x8A, 0x57, 0x1E, 0x03, 0xAC, 0x9C, 0x9E, 0xB7, 0x6F, 0xAC, 0x45, 0xAF, 0x8E, 0x51, 0x30, 0xC8, 0x1C, 0x46, 0xA3, 0x5C, 0xE4, 0x11, 0xE5, 0xFB, 0xC1, 0x19, 0x1A, 0x0A, 0x52, 0xEF, 0xF6, 0x9F, 0x24, 0x45, 0xDF, 0x4F, 0x9B, 0x17, 0xAD, 0x2B, 0x41, 0x7B, 0xE6, 0x6C, 0x37, 0x10]
        let plaintext = AESDecryptECB128(cyphertext: cyphertext, key: key)
        XCTAssertEqual(plaintext, expectedPlaintext)
    }
}
